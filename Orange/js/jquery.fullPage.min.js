	...
	b.fn.fullpage = function(c) {
		...
		function ua(a) {
			...
			if(!Z(a.target)) {
				...
				var e = Y(a);
				p || w || (d = aa(d), u = d.y, C = d.x, a.find(".fp-slides").length && Math.abs(D - C) > Math.abs(x - u) ? Math.abs(D - C) > b(window).width() /
					100 * c.touchSensitivity && (D > C ? m.right && b.fn.fullpage.moveSlideRight() : m.left && b.fn.fullpage.moveSlideLeft()) : c.autoScrolling && Math.abs(x - u) > b(window).height() / 100 * c.touchSensitivity && (x > u ? B("down", e) : u > x && B("up", e)))
			}
		}
		...
		function r(a, d, e) {
			...
			if("undefined" !== typeof f && (d = {
					...
					localIsResizing: v
				}, !(d.activeSection.is(a) && !v || c.scrollBar && b(window).scrollTop() ===
					d.dtop))) {
				...
				c.autoScrolling && c.continuousVertical && "undefined" !== typeof d.isMovementUp && (!d.isMovementUp && "up" == d.yMovement || d.isMovementUp && "down" == d.yMovement) && (d.isMovementUp ? b(".fp-section.active").before(d.activeSection.nextAll(".fp-section")) : b(".fp-section.active").after(d.activeSection.prevAll(".fp-section").get().reverse()), k(b(".fp-section.active").position().top), ca(), d.wrapAroundElements = d.activeSection,
					d.dest = d.element.position(), d.dtop = d.dest.top, d.yMovement = I(d.element));
				a.addClass("active").siblings().removeClass("active");
				p = !0;
		...
		function ha(a) {
			var b = "all " +
				c.scrollingSpeed + "ms " + c.easingcss3;
		...
		function z(a) {
			...
			b = l - parseInt(b.css("padding-bottom")) -
		...
		function O(a, d) {
			...
			c = isNaN(a) ? b('[data-anchor="' + a + '"]') : b(".fp-section").eq(a -
				1);
		...
		function aa(a) {
			...
			b.x = "undefined" !== typeof a.pageX &&
				(a.pageY || a.pageX) ? a.pageX : a.touches[0].pageX;
			return b
		}
		...
		function G(a, b) {
			console && console[a] && console[a]("fullPage: " +
				b)
		}
		c = b.extend({
			menu: !1,
		...
		(function() {
			...
			c.continuousVertical &&
				c.scrollBar && (c.continuousVertical = !1, G("warn", "Option `scrollBar` is mutually exclusive with `continuousVertical`; `continuousVertical` disabled"));
		...
		b(".fp-section").each(function(a) {
			...
			if(a.length) {
				...
				!c.animateAnchor && d.length &&
					(c.autoScrolling ? k(d.position().top) : (k(0), F(a), b("html, body").scrollTop(d.position().top)), K(a, null), b.isFunction(c.afterLoad) && c.afterLoad.call(d, a, d.index(".fp-section") + 1), d.addClass("active").siblings().removeClass("active"))
			}
		...
		b(document).keydown(function(a) {
			if(c.keyboardScrolling && c.autoScrolling &&
				(40 != a.which && 38 != a.which || a.preventDefault(), !p)) switch(a.which) {
				case 38:
				case 33:
